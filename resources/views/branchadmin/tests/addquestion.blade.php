                        ...
                        @if($section->id == '1')
                            @if(count($readingQuestions) > 0)
                                @foreach($readingQuestions as $readingQuestion)
                                    {!! Form::open(array('route' => 'branchadmin-tests-addQuestions','method'=>'POST','class'=>'form mt-4 ml-3')) !!}
                                    ...
                                    {!! Form::close() !!}
                                    <!-- <a href="{{ route('branchadmin-tests-addQuestions',['section_id'=>$section->id,'test_id'=>$pageArray[5],'question_type_id'=>$readingQuestion->id]) }}"><button type="button" class="btn mb-1 btn-primary btn-lg btn-block">{{ $readingQuestion->question_title }}</button></a> -->
                                @endforeach
                            @endif
                        @elseif ($section->id == '2')
                            @if(count($listeningQuestions) > 0)
                                @foreach($listeningQuestions as $listeningQuestion)
                                    ...
                                    {!! Form::close() !!}
                                    <!-- <a href="{{ route('branchadmin-tests-addQuestions',['section_id'=>$section->id,'test_id'=>$pageArray[5],'question_type_id'=>$listeningQuestion->id]) }}"><button type="button" class="btn mb-1 btn-primary btn-lg btn-block">{{ $listeningQuestion->question_title }}</button></a> -->
                                @endforeach
                            @endif
                        @elseif ($section->id == '3')
                            @if(count($writingQuestions) > 0)
                                @foreach($writingQuestions as $writingQuestion)
                                    ...
                                    <!-- <a href="{{ route('branchadmin-tests-addQuestions',['section_id'=>$section->id,'test_id'=>$pageArray[5],'question_type_id'=>$writingQuestion->id]) }}">
                        ...
                        @else
                            @if(count($speakingQuestions) > 0)
                                @foreach($speakingQuestions as $speakingQuestion)
                                    ...
                                    {!! Form::close() !!}
                                    <!-- <a href="{{ route('branchadmin-tests-addQuestions',['section_id'=>$section->id,'test_id'=>$pageArray[5],'question_type_id'=>$speakingQuestion->id]) }}">
