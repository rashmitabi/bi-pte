  ...
  <form class="form mt-5" method="POST">
    ...
    <div class="form-group row">
      <div class="col-12 col-md-12 col-xl-12 col-sm-12 save-btn">
        @if(isset($user->id))
          <button  type="button" class="btn btn-outline-primary user-password-update" data-id="{{ $user->id }}" data-url="{{ route('branchadmin-students-setpassword', $user->id) }}"><i class="far fa-save save-icon"></i>Save Password</button>
        @else
          @foreach($user as $row)
            <input type="hidden" name="user_ids[]" value="{{$row->id}}">
          @endforeach
          <input type="hidden" name="role_id" value="{{$user[0]->role_id}}">
          <button  type="button" class="btn btn-outline-primary user-password-update" data-id="" data-url="{{ route('branchadmin-students-setpassword', $user[0]->id) }}"><i class="far fa-save save-icon"></i>Save Password</button>
        @endif
